 ...
 * updated to that of PuTTY 0.49 beta
 */
...
void random_block(void *block, int blockSize)
{
  int i=0;
  int size=0;
  char *bufpos=block;
  ...
  while (blockSize>0)
  {
    if (pool.poolpos<POOLSIZE)
    {
      size = (POOLSIZE-pool.poolpos);
      if (size>blockSize)
        size=blockSize;
      memcpy(bufpos,&(pool.pool[pool.poolpos]),size);
      blockSize -= size;
      bufpos += size;
    }
    random_stir();
...
unsigned int random_word(void)
{
  ...
  int size=0;
    ...
    if (pool.poolpos+4 >= POOLSIZE)
	random_stir();

    return (unsigned int ) ((pool.pool[pool.poolpos++]<<24) |
            (pool.pool[pool.poolpos++]<<16) |
            (pool.pool[pool.poolpos++]<<8)  |
            (pool.pool[pool.poolpos++]<<0));
